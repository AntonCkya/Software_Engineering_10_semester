...
from app.storage import delivery_repository, user_repository, parcel_repository
...
async def create_delivery(
    request: DeliveryCreate, current_user: User = Depends(get_current_user)
):
    ...
    sender = user_repository.get_by_id(request.sender_id)
    if not sender:
        raise HTTPException(
    ...
    recipient = user_repository.get_by_id(request.recipient_id)
    if not recipient:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    parcel = parcel_repository.get_by_id(request.parcel_id)
    if not parcel:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    db_delivery = delivery_repository.create(delivery)
...
async def get_delivery(
    delivery_id: str, current_user: User = Depends(get_current_user)
):
    delivery = delivery_repository.get_by_id(delivery_id)
    if not delivery:
        raise HTTPException(
...
async def get_deliveries_by_sender(
    sender_id: str, current_user: User = Depends(get_current_user)
):
    sender = user_repository.get_by_id(sender_id)
    if not sender:
        raise HTTPException(
    ...
    deliveries = delivery_repository.get_by_sender_id(sender_id)
...
async def get_deliveries_by_recipient(
    recipient_id: str, current_user: User = Depends(get_current_user)
):
    recipient = user_repository.get_by_id(recipient_id)
    if not recipient:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    deliveries = delivery_repository.get_by_recipient_id(recipient_id)
