...
from app.storage import parcel_repository, user_repository
...
async def create_parcel(
    request: ParcelCreate, current_user: User = Depends(get_current_user)
):
    owner = user_repository.get_by_id(request.owner_id)
    if not owner:
        raise HTTPException(
    ...
    db_parcel = parcel_repository.create(parcel)
...
async def get_parcel(parcel_id: str, current_user: User = Depends(get_current_user)):
    parcel = parcel_repository.get_by_id(parcel_id)
    if not parcel:
        raise HTTPException(
...
async def get_user_parcels(
    ...
    parcels = parcel_repository.get_by_owner_id(user_id)
...
async def get_parcel_by_tracking(
    tracking_number: str, current_user: User = Depends(get_current_user)
):
    parcel = parcel_repository.get_by_tracking_number(tracking_number)
    if not parcel:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
